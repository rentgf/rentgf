...
export default function RegisterPage() {
  ...
  async function handleRegister(event: React.FormEvent) {
    ...
    const { data, error: authError } = await supabase.auth.signUp({
      email,
      password,
      options: { data: { display_name: displayName, date_of_birth: dob } },
    })
    ...
    return (
      <main className="flex min-h-screen items-center justify-center bg-[#fbfaf7] px-4 py-10">
        <div className="w-full max-w-md rounded-3xl border border-[#cfe2d3] bg-[#f4faf4] p-8">
          ...
          <button type="button" onClick={() => router.push('/login')} className="mt-6 inline-flex rounded-xl bg-[#173f35] px-5 py-3 text-sm font-semibold text-white">Go to login</button>
        </div>
      </main>
    )
  }
      ...
      <div className="mx-auto max-w-md">
        <div className="rounded-3xl border border-[#e9e2d9] bg-white p-6 shadow-sm sm:p-8">
          ...
          <form onSubmit={handleRegister} className="mt-8 flex flex-col gap-4">
            <label className="text-sm font-medium">Display name<input required value={displayName} onChange={(e) => setDisplayName(e.target.value)} placeholder="How you want to be known" className="mt-2 w-full rounded-xl border border-[#e5e1da] px-3 py-3 outline-none focus:ring-2 focus:ring-[#bdd2c7]" /></label>
            <label className="text-sm font-medium">Email<input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@example.com" className="mt-2 w-full rounded-xl border border-[#e5e1da] px-3 py-3 outline-none focus:ring-2 focus:ring-[#bdd2c7]" /></label>
            <label className="text-sm font-medium">Password<input type="password" minLength={8} required value={password} onChange={(e) => setPassword(e.target.value)} className="mt-2 w-full rounded-xl border border-[#e5e1da] px-3 py-3 outline-none focus:ring-2 focus:ring-[#bdd2c7]" /></label>
            <label className="text-sm font-medium">Date of birth<input type="date" required value={dob} onChange={(e) => setDob(e.target.value)} className="mt-2 w-full rounded-xl border border-[#e5e1da] px-3 py-3 outline-none focus:ring-2 focus:ring-[#bdd2c7]" /></label>
            ...
            {error && <p className="rounded-xl bg-[#fff2ed] px-4 py-3 text-sm text-[#9e4f38]" role="alert">{error}</p>}
