...
export default function EarnAsCompanionPage() {
  ...
  async function sendCode() {
    ...
    if (!response.ok || !result.ok) {
      throw new Error(result.error ?? 'Could not send the verification code. Please try again.')
    }
  }
      ...
      <div className="mx-auto max-w-xl">
        ...
        <section className="mt-10 rounded-[28px] border border-[#e9e2d9] bg-white p-6 shadow-[0_16px_50px_rgba(23,63,53,.06)] sm:p-9">
          {step === 'created' ? (
            ...
            <>
              ...
              <form onSubmit={handleVerify} className="mt-7 flex flex-col gap-4">
                <label className="text-sm font-medium">
                  Verification code
                  <input type="text" inputMode="numeric" pattern="[0-9]{6}" maxLength={6} required autoFocus value={otp}
              ...
              <div className="mt-5 flex items-center justify-between text-sm text-[#68756e]">
                ...
                {resendCooldown > 0 ? <span className="text-[#9aa49d]">Resend in {resendCooldown}s</span> : (
            ...
            <>
              <div className="flex size-12 items-center justify-center rounded-2xl bg-[#edf4ee]"><HeartHandshake className="size-6 text-[#4e8068]" /></div>
              ...
              <p className="mt-3 text-sm leading-6 text-[#68756e]">Create your companion account here, then submit a profile for review. Customer sign-up and customer bookings stay separate.</p>

              <div className="mt-6 grid gap-3 sm:grid-cols-2">
                <div className="rounded-2xl bg-[#f5f3ef] p-4"><ShieldCheck className="size-5 text-[#4e8068]" /><p className="mt-3 text-sm font-semibold">Reviewed before going live</p><p className="mt-1 text-xs leading-5 text-[#68756e]">Your companion profile goes through a separate review.</p></div>
                <div className="rounded-2xl bg-[#f5f3ef] p-4"><Sparkles className="size-5 text-[#c36d4d]" /><p className="mt-3 text-sm font-semibold">Your own companion space</p><p className="mt-1 text-xs leading-5 text-[#68756e]">Manage requests and companion activity in your studio.</p></div>
              </div>

              <form onSubmit={handleRegister} className="mt-7 flex flex-col gap-4">
                <label className="text-sm font-medium">Name for your account<input required maxLength={80} value={displayName} onChange={(event) => setDisplayName(event.target.value)} placeholder="e.g. Priya S." className="mt-2 w-full rounded-xl border border-[#e5e1da] px-3 py-3 outline-none focus:ring-2 focus:ring-[#bdd2c7]" /></label>
                <label className="text-sm font-medium">Email<input type="email" required value={email} onChange={(event) => setEmail(event.target.value)} placeholder="you@example.com" className="mt-2 w-full rounded-xl border border-[#e5e1da] px-3 py-3 outline-none focus:ring-2 focus:ring-[#bdd2c7]" /></label>
                <label className="text-sm font-medium">Password<input type="password" minLength={8} required value={password} onChange={(event) => setPassword(event.target.value)} className="mt-2 w-full rounded-xl border border-[#e5e1da] px-3 py-3 outline-none focus:ring-2 focus:ring-[#bdd2c7]" /><span className="mt-1 block text-xs text-[#8a958e]">Use at least 8 characters.</span></label>
                <label className="text-sm font-medium">Date of birth<input type="date" required value={dob} onChange={(event) => setDob(event.target.value)} className="mt-2 w-full rounded-xl border border-[#e5e1da] px-3 py-3 outline-none focus:ring-2 focus:ring-[#bdd2c7]" /></label>
                <label className="flex items-start gap-3 text-sm leading-5 text-[#68756e]"><input type="checkbox" checked={ageConfirmed} onChange={(event) => setAgeConfirmed(event.target.checked)} className="mt-1" /><span>I confirm that I am 18 or older and agree to the <Link href="/terms" className="underline hover:text-[#173f35]">Terms of Service</Link> and <Link href="/privacy-policy" className="underline hover:text-[#173f35]">Privacy Policy</Link>.</span></label>
                {error && <p role="alert" className="rounded-xl bg-[#fff2ed] px-4 py-3 text-sm text-[#9e4f38]">{error}</p>}
                <button type="submit" disabled={loading} className="rounded-full bg-[#173f35] px-5 py-4 font-semibold text-white transition hover:bg-[#245447] disabled:opacity-60">{loading ? 'Sending verification code…' : 'Create companion account'}</button>
              </form>
              <p className="mt-5 text-center text-sm text-[#68756e]">Already have an account? <Link href="/login?redirectTo=/become-companion" className="font-semibold text-[#c36d4d]">Sign in to apply</Link></p>
              <p className="mt-3 text-center text-xs text-[#8a958e]">Looking to book a companion instead? <Link href="/register" className="font-semibold text-[#c36d4d]">Customer sign-up</Link></p>
            </>
          )}
        </section>
      </div>
